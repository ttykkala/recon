...
//    integer ncola = 0;
	...
	if (info != 0) {
                xerbla2_((char*)"ZGEMM ", &info, (ftnlen)6);
		return 0;
	}
...
//		printf("d__1: %3.1f, *t == %f",d__1,*t,hnorm,abs(d__1));
	...
	*ns = max(i__1,i__2);
	...
	/* ---  H2 = scale2*H*H ... */
        ...
        zgemm_((char*)"n", (char*)"n", m, m, m, &scale2, &h__[h_offset], ldh, &h__[h_offset],
...
L100:
	iused = iodd * iq + (1 - iodd) * ip;
        zgemm_((char*)"n", (char*)"n", m, m, m, &c_b173, &wsp[iused], m, &wsp[ih2], m, &c_b172,
	...
	if (iodd != 0) {
                zgemm_((char*)"n", (char*)"n", m, m, m, &scale, &wsp[iq], m, &h__[h_offset], ldh, &
			c_b172, &wsp[ifree], m, (ftnlen)1, (ftnlen)1);
		iq = ifree;
	} else {
                zgemm_((char*)"n", (char*)"n", m, m, m, &scale, &wsp[ip], m, &h__[h_offset], ldh, &
			c_b172, &wsp[ifree], m, (ftnlen)1, (ftnlen)1);
		ip = ifree;
	}
	z__1.r = -1., z__1.i = -0.;
	...
	for (k = 1; k <= i__1; ++k) {
		...
		iput = (1 - iodd) * ip + iodd * iq;
                zgemm_((char*)"n", (char*)"n", m, m, m, &c_b173, &wsp[iget], m, &wsp[iget], m, &
...
__device__ void normalizeMat4RealSafe(doublereal *T) {
    // extract column vectors SO3
    doublereal u[3],v[3],w[3];
    u[0] = T[0]; u[1] = T[4]; u[2] = T[8];
    v[0] = T[1]; v[1] = T[5]; v[2] = T[9];
...
__global__ void expmKernel(doublereal *mtx, float *TF, doublereal scale)
{
    ...
    if (maxEntry != 0) {
        ...
        if (flag != 0) {
            ...
            for (int i = 0; i < m; i++) Tinc[i+i*m] = 1;
        } else {
...
extern "C" void expmCuda(doublereal * mtxDev, float * outputDev, float scaleOut, cudaStream_t stream)
{
    assert(mtxDev != NULL && outputDev != NULL);
    dim3 cudaBlockSize(1,1,1);
    dim3 cudaGridSize(1,1,1);
    expmKernel<<<cudaGridSize,cudaBlockSize,0,stream>>>(mtxDev,outputDev,doublereal(scaleOut));
    getLastCudaError("expmKernel() execution failed\n");
}
